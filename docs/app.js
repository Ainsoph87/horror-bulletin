...
function renderCards() {
  ...
  document.getElementById('cards-container').innerHTML = filtered.map(d => {
    ...
    // niente più approvazione: ogni voce non scartata è postabile (dalla pagina Social).
    // Sulla card resta solo lo scarto: rimuove la voce ovunque. La selezione ZIP vive tutta
    // sulla pagina Social (lista + Tutte/Nessuna), non serve duplicarla qui.
    const postBtn = `<div class="card-actions">
        <button class="card-btn card-reject" title="Non è horror / non vale: rimuovi ovunque" onclick='HB.reject(${JSON.stringify(d.id)})'>🗑 Elimina</button>
      </div>`;
    return `
      <div class="card">
...
function renderArchive() {
  ...
  if (!DATA.items.length) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty">Nessun dato disponibile.</td></tr>';
    return;
  }
  tbody.innerHTML = [...DATA.items].sort(byCategory).map(d => `
    <tr>
      ...
      <td><button class="card-btn card-reject" title="Rimuovi ovunque" onclick='HB.reject(${JSON.stringify(d.id)})'>🗑</button></td>
    </tr>
  `).join('');
}
...
function showPage(id) {
  ...
  const tabs = { bulletin: 0, archivio: 1, social: 2, guida: 3 };
...
// ── Scarto voce: opt-out via Cloudflare Worker (flag Scartato su Notion) ──
// Sostituisce la vecchia pagina Review: niente approvazione, solo rimozione dei falsi positivi.
const WORKER_URL = localStorage.getItem('hb_worker_url') || 'https://horror-bulletin-api.di-rodi-mirko2.workers.dev';

function apiKey() {
  let k = localStorage.getItem('hb_api_key');
  if (!k) {
    k = (prompt('API Key del worker (si salva nel browser):', '') || '').trim();
    if (k) localStorage.setItem('hb_api_key', k);
  }
  return k;
}
...
async function reject(id) {
  ...
  if (!item) return;
  if (!confirm(`Rimuovere "${item.title}" da sito, archivio, bulk e social?\n(reversibile togliendo la spunta Scartato su Notion)`)) return;
  const key = apiKey();
  if (!key) { showToast('⚠️ Serve la API Key del worker'); return; }
  try {
    const r = await fetch(WORKER_URL + '/update', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Api-Key': key },
      body: JSON.stringify({ id, props: { scartato: true } })
    });
    const json = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(json.error || `HTTP ${r.status}`);
    // rimozione ottimistica: sparisce subito ovunque, il sync la conferma definitivamente
    DATA.items = DATA.items.filter(i => i.id !== id);
    renderStats(); renderCards(); renderArchive();
    if (window.HBSocial) HBSocial.init(DATA);
...
window.HB = { get DATA() { return DATA; }, monthItems, formatDate, showPage, showToast, reject, CAT_BADGE, byCategory, catRank, CAT_ORDER };
